...
pub fn execute(app: &mut App, cmd: &Command) {
    ...
    if browsing_output {
        let handled = match cmd {
            Command::MoveLeft => Some(notebook::output_motion(app, extend, motion::move_left)),
            Command::MoveRight => Some(notebook::output_motion(app, extend, motion::move_right)),
            Command::MoveWordForward => Some(notebook::output_motion(app, extend, motion::move_word_forward)),
            Command::MoveWordBackward => Some(notebook::output_motion(app, extend, motion::move_word_backward)),
            Command::MoveWordEnd => Some(notebook::output_motion(app, extend, motion::move_word_end)),
            Command::MoveBigWordForward => Some(notebook::output_motion(app, extend, motion::move_big_word_forward)),
            Command::MoveBigWordBackward => Some(notebook::output_motion(app, extend, motion::move_big_word_backward)),
            Command::MoveBigWordEnd => Some(notebook::output_motion(app, extend, motion::move_big_word_end)),
            Command::MoveLineStart => Some(notebook::output_motion(app, extend, motion::move_line_start)),
            Command::MoveLineFirstNonWs => Some(notebook::output_motion(app, extend, motion::move_line_first_non_ws)),
            Command::MoveLineEnd => Some(notebook::output_motion(app, extend, motion::move_line_end)),
            _ => None,
        };
    ...
    match cmd {
        ...
        Command::MoveUp => {
            ...
            if (!extend || extending_output) && notebook::notebook_vertical(app) && notebook::notebook_move_up(app, extend) {
                return;
            }
        ...
        Command::MoveDown => {
            ...
            let extending_output = extend && browsing_output;
            if (!extend || extending_output) && notebook::notebook_vertical(app) && notebook::notebook_move_down(app, extend) {
                return;
            }
        ...
        Command::YankSelection => {
            if browsing_output {
                if let Some(text) = notebook::yank_output_selection(app) {
        ...
        Command::Write | Command::WriteForce => {
            buffers::write_buffer(app, matches!(cmd, Command::WriteForce));
            return;
        }
        ...
        Command::BufferClose | Command::BufferForceClose => {
            buffers::close_buffer(app, matches!(cmd, Command::BufferForceClose));
            return;
        }
        ...
        Command::NotebookNextCell => {
            let target = app.notebook.as_ref()
                ...
                .unwrap_or(0);
            notebook::switch_focused_cell(app, target);
            return;
        }
        ...
        Command::NotebookGotoError => {
            notebook::goto_focused_cell_error(app);
            return;
        }
        Command::NotebookFollowError => {
            notebook::follow_output_error_link(app);
            return;
        }
...
mod tests {
    ...
    fn async_kernel_executes_queued_cells_in_order() {
        ...
        loop {
            notebook::process_kernel_events(&mut app);
    ...
    fn the_kernel_bridge_lists_the_namespace_and_fetches_by_name() {
        ...
        let pump = |app: &mut App, until: &dyn Fn(&App) -> bool, what: &str| {
            let deadline = std::time::Instant::now() + std::time::Duration::from_secs(60);
            while !until(app) {
                notebook::process_kernel_events(app);
    ...
    fn kernel_error_frame_resolves_and_goto_error_jumps() {
        ...
        // The error output carries a navigable frame → cell 0, line 2.
        {
            ...
            assert_eq!(notebook::resolve_error_frame(nb, inner), Some((0, 2)));
        }
    ...
    fn expanded_output_exposes_every_row_to_navigation() {
        ...
        assert_eq!(notebook::nb_output_rows(&app, 0), cap + 1);

        execute(&mut app, &Command::NotebookToggleOutputExpand);
        assert!(app.notebook.as_ref().unwrap().1.is_output_expanded(0));
        assert_eq!(notebook::nb_output_rows(&app, 0), total, "expanding reveals every line");
